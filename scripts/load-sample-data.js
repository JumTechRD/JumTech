// Script para cargar datos de ejemplo en todas las secciones administrativas
...
async function loadAllSampleData() {
  ...
  console.log(`- Cotizaciones: ${cotizacionesEjemplo.length}`);
  console.log(`- Facturas: ${facturasEjemplo.length}`);
  console.log("\n💰 Totales estimados:");
  console.log(`- Valor total de inventario: RD$ ${productosEjemplo.reduce((sum, p) => sum + (p.precio * p.stock), 0).toLocaleString()}`);
  console.log(`- Total de cotizaciones: RD$ ${cotizacionesEjemplo.reduce((sum, c) => sum + c.total, 0).toLocaleString()}`);
  console.log(`- Total de facturas: RD$ ${facturasEjemplo.reduce((sum, f) => sum + f.total, 0).toLocaleString()}`);
}
